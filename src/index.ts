...
try {
  ...
  try {
    ...
    if (!process.env.MCP_INSPECTOR) {
      originalConsole.log(`Sectors MCP server (${SERVER_CONFIG.name} v${SERVER_CONFIG.version}) started successfully`);
    }
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Failed to initialize MCP server';
