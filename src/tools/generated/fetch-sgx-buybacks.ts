...
export async function fetchSgxBuybacks(
  baseUrl: string,
  ...
  const url = new URL(`${baseUrl}/sgx/buybacks/`);
...
export function registerFetchSgxBuybacksTool(
  server: McpServer,
  baseUrl: string,
  apiKey: string | undefined
) {
  server.registerTool(
    "fetch-sgx-buybacks",
    {
      ...
      start: z.string()
        .describe("Start date in `YYYY-MM-DD` format. Optional; if omitted, no lower bound is applied. Filters on `purchase_date`.").optional(),
      end: z.string()
